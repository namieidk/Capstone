...
function getInitials(firstName?: string, lastName?: string): string {
  const f = firstName?.[0] ?? "";
  const l = lastName?.[0] ?? "";
  return (f + l).toUpperCase() || COORDINATOR.initials || "PR";
}

interface CoordinatorSidebarProps extends React.ComponentProps<typeof Sidebar> {}

export function CoordinatorSidebar({ ...props }: CoordinatorSidebarProps) {
        ...
        <SidebarGroup>
          <SidebarGroupContent>
            <SidebarMenu className="gap-1">
              {COORDINATOR_NAV_ITEMS.filter((item) => !item.hidden).map((item) => {
        ...
        <div className="flex items-center justify-between gap-2 rounded-lg bg-white/5 p-2 transition-colors group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:bg-transparent group-data-[collapsible=icon]:p-0">
          ...
          >
            ...
            <div className="flex min-w-0 flex-col group-data-[collapsible=icon]:hidden">
              ...
              <span className="truncate text-[0.72rem] text-white/60 leading-tight">Coordinator</span>
            </div>
          </Link>
