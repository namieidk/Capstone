...
export function ContactFormCard() {
  ...
  return (
    <div className="rounded-[20px] border border-line bg-card p-8 shadow-va-sm transition-shadow duration-300 hover:shadow-va-md">
      <h3 className="font-serif text-[1.5rem] font-medium text-navy">Send us a message</h3>
      <p className="mt-1 mb-6 text-[0.92rem] text-muted-foreground">We will get back to you by email.</p>

      {sent ? (
        <div className="animate-in fade-in zoom-in-95 rounded-xl bg-good-bg px-5 py-4 text-[0.95rem] font-semibold text-good duration-300">
        ...
        >
          ...
          <Button
            type="submit"
            className="mt-1 h-10 w-fit self-start rounded-full px-6 text-[0.94rem] font-semibold transition-transform duration-300 hover:-translate-y-0.5 hover:shadow-va-md"
          >
            Send message
          </Button>
        </form>
...
export function Contact() {
  return (
    <section id="contact" className="relative overflow-hidden bg-background py-19">
      <Fireflies />
            ...
            <div className="flex flex-col gap-4.5">
              {CONTACT_INFO.map((c) => (
                <div key={c.label} className="flex gap-4">
                  <span className="flex size-10 shrink-0 items-center justify-center rounded-full border border-line bg-card text-navy">
