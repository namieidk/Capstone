...
import { GRANTOR_NAV_ITEMS } from "@/components/app-sidebar/nav-grantor";
import { useStaffBadges } from "@/components/app-sidebar/useStaffBadges";
import { GRANTOR } from "@/components/Grantorshared";
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
...
function getInitials(firstName?: string, lastName?: string): string {
  const f = firstName?.[0] ?? "";
  const l = lastName?.[0] ?? "";
  return (f + l).toUpperCase() || GRANTOR.initials || "CR";
}

interface GrantorSidebarProps extends React.ComponentProps<typeof Sidebar> {}

export function GrantorSidebar({ ...props }: GrantorSidebarProps) {
  const pathname = usePathname();
  const { user, logout } = useAuth();

  const displayName = user ? `${user.first_name} ${user.last_name}` : GRANTOR.name;
  const displayInitials = user ? getInitials(user.first_name, user.last_name) : GRANTOR.initials;
  const liveBadges = useStaffBadges({ includeApplicants: false });
        ...
        <SidebarGroup>
          <SidebarGroupContent>
            <SidebarMenu className="gap-1">
              {GRANTOR_NAV_ITEMS.filter((item) => !item.hidden).map((item) => {
                const Icon = item.icon;
                const isActive =
                  pathname === item.href || (item.href !== "/grantDashboard" && pathname.startsWith(item.href));
                const badge = item.key === "meeting" ? liveBadges.meetings : item.badge;
        ...
        <div className="flex items-center justify-between gap-2 rounded-lg bg-white/5 p-2 transition-colors group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:bg-transparent group-data-[collapsible=icon]:p-0">
          ...
          >
            ...
            <div className="flex min-w-0 flex-col group-data-[collapsible=icon]:hidden">
              ...
              <span className="truncate text-[0.72rem] text-white/60 leading-tight">Grantor</span>
            </div>
          </Link>
