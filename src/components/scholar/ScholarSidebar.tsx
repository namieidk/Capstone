...
import { SCHOLAR_NAV_ITEMS } from "@/components/app-sidebar/nav-scholar";
import { SCHOLAR } from "@/components/ScholarShared";
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
...
function getInitials(firstName?: string, lastName?: string): string {
  const f = firstName?.[0] ?? "";
  const l = lastName?.[0] ?? "";
  return (f + l).toUpperCase() || SCHOLAR.initials || "DF";
}

interface ScholarSidebarProps extends React.ComponentProps<typeof Sidebar> {}

export function ScholarSidebar({ ...props }: ScholarSidebarProps) {
  const pathname = usePathname();
  const { user, logout } = useAuth();

  const displayName = user ? `${user.first_name} ${user.last_name}` : SCHOLAR.name;
  const displayInitials = user ? getInitials(user.first_name, user.last_name) : SCHOLAR.initials;
        ...
        <div className="flex items-center justify-between gap-2 rounded-lg bg-white/5 p-2 transition-colors group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:bg-transparent group-data-[collapsible=icon]:p-0">
          ...
          >
            ...
            <div className="flex min-w-0 flex-col group-data-[collapsible=icon]:hidden">
              ...
              <span className="truncate text-[0.72rem] text-white/60 leading-tight">Scholar</span>
            </div>
          </Link>
