...
export function GrantorSidebar({ mobileOpen }: GrantorSidebarProps) {
  const pathname = usePathname();

  return (
    <aside className={`vg-sidebar ${mobileOpen ? "is-open" : ""}`} style={s.sidebar}>
      ...
      <div style={s.sidebarUserCard}>
        <span style={s.sidebarAvatar}>{GRANTOR.initials}</span>
        <div style={s.sidebarUserInfo}>
          <p style={s.sidebarUserName}>{GRANTOR.name}</p>
          <p style={s.sidebarUserRole}>Grantor</p>
        </div>
...
interface GrantorTopBarProps {
  onMenuClick: () => void;
  title: string;
  subtitle: string;
}

export function GrantorTopBar({ onMenuClick, title, subtitle }: GrantorTopBarProps) {
  return (
    <header style={s.topbar}>
      <button className="vg-mobile-toggle" onClick={onMenuClick} style={s.mobileToggle}>
        <MenuIcon />
      </button>
      <div>
      ...
      <div style={s.topbarRight}>
        <div className="vg-topbar-search" style={s.searchBox}>
          <SearchIcon />
          <input placeholder="Search scholars..." style={s.searchInput} />
        </div>
