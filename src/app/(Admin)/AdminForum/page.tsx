...
function findProfile(author: string) {
  const scholar = SCHOLARS_DIRECTORY.find((s) => s.name === author);
  if (scholar) return { email: scholar.email, phone: scholar.phone, detail: `${scholar.course}, ${scholar.year}` };
  const employee = EMPLOYEES.find((e) => e.name === author);
  if (employee) return { email: employee.email, phone: null as string | null, detail: employee.department };
  if (author === ADMIN.name) return { email: ADMIN.email, phone: null as string | null, detail: ADMIN.title };
  return null;
}
...
export default function AdminForumPage() {
  ...
  const submitReply = (postId: number) => {
    ...
    if (!text) return;
    const reply: Reply = { id: Date.now(), author: ADMIN.name, initials: ADMIN.initials, text, time: "Just now" };
  ...
  const selectedProfile = selectedPost ? findProfile(selectedPost.author) : null;
  const others = posts.filter((p) => p.id !== selectedPost?.id);

  return (
    <div style={{ maxWidth: 1040, margin: "0 auto", padding: "28px 4px 60px" }}>
      {/* ---- Two-column layout ---- */}
      <div
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {posts.map((post) => {
            ...
            const stackAuthors = posts.filter((p) => p.id !== post.id).slice(0, 3);
            const extraCount = Math.max(0, posts.length - 1 - stackAuthors.length);
