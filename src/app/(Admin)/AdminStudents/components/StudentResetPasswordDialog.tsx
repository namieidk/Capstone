...
interface StudentResetPasswordDialogProps {
  ...
  onDirectReset: (studentId: number, newPass: string) => Promise<void>;
...
export function StudentResetPasswordDialog({
  ...
  const handleConfirmAction = async () => {
    ...
    if (pendingAction === "DIRECT") {
      await onDirectReset(student.id, newPassword);
      setNewPassword("");
      onOpenChange(false);
    } else if (pendingAction === "EMAIL") {
      await onSendResetEmail(student.email);
      setEmailSent(true);
    }
    ...
    <>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription className="text-xs text-muted-foreground pt-0.5">
                  Manage login credentials for <strong>{student.name}</strong> ({student.type})
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
          ...
          {emailSent ? (
            <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4 text-center space-y-2">
              ...
              <p className="text-[11px] text-muted-foreground">
                A secure password reset link has been sent to <strong>{student.email}</strong>.
              </p>
              <div className="pt-2">
            ...
            <div className="space-y-4 pt-2">
              ...
              <form onSubmit={handleOpenDirectConfirm} className="space-y-3">
                <div className="space-y-1.5">
                  <Label htmlFor="student-new-password" className="text-xs font-semibold">
      ...
      <AlertDialog open={showConfirmAlert} onOpenChange={setShowConfirmAlert}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {pendingAction === "EMAIL" ? "Send password reset email?" : "Reset student password?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {pendingAction === "EMAIL"
                ? `A password reset link will be emailed to ${student.email}. They will be able to choose a new password.`
                : `Are you sure you want to set a new password for ${student.name}? Their existing password will be overridden immediately.`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
