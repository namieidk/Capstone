...
import type { GrantorScholarHealthItem } from "@/lib/api/grantor-dashboard";

interface GrantorScholarHealthCardProps {
  scholars: GrantorScholarHealthItem[];
}

export function GrantorScholarHealthCard({ scholars }: GrantorScholarHealthCardProps) {
  const getHealthBadge = (health: "good" | "warn" | "bad") => {
    if (health === "warn") {
      return (
    ...
    <Card className="flex flex-col justify-start rounded-2xl border-line/80 bg-white shadow-2xs hover:shadow-xs transition-all">
      <CardHeader className="flex flex-row items-center justify-between pb-3 p-5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm font-bold tracking-tight text-[#14213a]">Scholar Program Retention & Standings</h2>
            <p className="text-xs text-muted-foreground">
              Academic baseline health, GWA standing & retention oversight
            </p>
          </div>
        </div>
      ...
      <CardContent className="p-5 pt-0 space-y-3">
        {scholars.length === 0 ? (
          <div className="rounded-xl border border-dashed border-line/70 p-6 text-center text-xs text-muted-foreground">
            No active scholars enrolled in the program.
          </div>
        ) : (
          scholars.map((sc) => (
            <div
