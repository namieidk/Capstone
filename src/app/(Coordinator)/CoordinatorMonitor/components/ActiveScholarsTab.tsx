...
import { ChevronLeft, ChevronRight, Eye, TrendingDown, TrendingUp } from "lucide-react";
...
export function ActiveScholarsTab({
  ...
  return (
    <Card className="mt-4 rounded-[18px]! border-line bg-white shadow-va-sm">
      <CardHeader className="pb-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm font-medium text-muted-foreground">
            {!loading && !loadError && (
              <>
                {filtered.length} {filtered.length === 1 ? "active scholar" : "active scholars"}
              </>
            )}
          </p>
        </div>
