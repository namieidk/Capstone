...
export default function MeetingPage() {
  ...
  const dayMeetings = combined.filter((m) => dateKey(m.dateObj) === selectedDateKey);
  ...
  return (
    <div>
      <style>{`
        .meeting-grid { display: grid; grid-template-columns: 1fr 328px; gap: 26px; align-items: start; margin-top: 28px; }
        @media (max-width: 980px) { .meeting-grid { grid-template-columns: 1fr !important; } }
      ...
      <div className="meeting-grid">
        {/* ---------------- Left: day timeline ---------------- */}
        <div style={{ background: WHITE, border: BORDER_SUBTLE, borderRadius: 20, boxShadow: SHADOW_SM, padding: "28px 30px 32px" }}>
          ...
          {dayMeetings.length === 0 ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "64px 0", color: "#9a9a94", gap: 10 }}>
              <CalendarIcon />
              <p style={{ fontSize: "0.92rem" }}>No interviews scheduled for this day.</p>
                  ...
                  return (
                    ...
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                        ...
                        <span style={{ width: 7, height: 7, borderRadius: "50%", background: GOOD, marginTop: 5, flexShrink: 0 }} />
                      </div>
                      ...
                      <button
                        ...
                        style={{ alignSelf: "flex-start", marginTop: 2, background: WHITE, color: NAVY, fontWeight: 600, fontSize: "0.72rem", padding: "4px 10px", borderRadius: 999, border: `1px solid ${LINE}` }}
                      >
                        Join
                      </button>
                    </div>
        ...
        <aside style={{ background: WHITE, border: BORDER_SUBTLE, borderRadius: 20, boxShadow: SHADOW_SM, padding: "24px 24px 26px", position: "sticky", top: 24 }}>
        ...
        <div style={s.drawerOverlay} onClick={() => setShowAllDrawer(false)}>
          <div style={s.drawerPanel} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 26 }}>
              {combined.length === 0 && <p style={{ fontSize: "0.86rem", color: "#9a9a94" }}>No interviews scheduled yet.</p>}
              {combined.map((m) => (
                <button
                  key={m.id}
                  onClick={() => {
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {passedApplicants.length === 0 && <p style={{ fontSize: "0.86rem", color: "#9a9a94" }}>Everyone who passed review has been scheduled.</p>}
              {passedApplicants.map((a) => (
                <button
                  key={a.id}
                  onClick={() => {
