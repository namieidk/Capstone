    ...
    <div className="rounded-xl border border-border bg-white p-4 shadow-xs">
      ...
      {gradeItems.length === 0 ? (
        ...
        <>
          ...
          <div className="hidden items-center gap-2 rounded-lg border border-border/60 bg-muted/60 px-2 py-1.5 text-[11px] font-bold uppercase tracking-wider text-[#6a6a64] sm:flex mb-2">
            <span className="w-5 shrink-0 text-center">#</span>
            {(hasCodes || !isReadOnly) && <span className="w-24 shrink-0 text-center">Code</span>}
            <span className="min-w-0 flex-1 text-left pl-1">Subject Title</span>
            {hasUnits && <span className="w-16 shrink-0 text-center">Units</span>}
            <span className="w-20 shrink-0 text-center">Grade</span>
            {!isReadOnly && <span className="w-5.5 shrink-0" />}
          </div>
              ...
              {gradeItems.map((item, index) => (
                <div key={item.id}>
                  ...
                  <div className="hidden items-center gap-2 rounded-lg border border-border bg-muted/40 p-2 text-xs sm:flex">
                    <span className="w-5 text-center text-[0.7rem] font-semibold text-muted-foreground">
                      {index + 1}
                    </span>
                    {(hasCodes || !isReadOnly) && (
                      <Input
