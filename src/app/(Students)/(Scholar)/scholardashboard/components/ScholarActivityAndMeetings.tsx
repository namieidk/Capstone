...
import { ArrowRight, Calendar, ExternalLink, Megaphone, MessageSquare, Video } from "lucide-react";
import Link from "next/link";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader } from "@/components/ui/card";
import type {
  ScholarAnnouncementItem,
  ScholarCommunicationData,
  ScholarMeetingItem,
} from "@/lib/api/scholar-dashboard";

interface ScholarActivityAndMeetingsProps {
  communication: ScholarCommunicationData;
  meetings: ScholarMeetingItem[];
  announcements: ScholarAnnouncementItem[];
}

export function ScholarActivityAndMeetings({
  communication,
  meetings,
  announcements,
}: ScholarActivityAndMeetingsProps) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {/* LEFT: Coordinator Messages & Upcoming Meetings */}
        ...
        <CardContent className="p-5 pt-0 space-y-4">
          ...
          <div className="flex items-center justify-between rounded-xl border border-line/60 bg-[#FAF8F5] p-3">
            ...
            <Button
              asChild
              size="sm"
              className="h-7.5 rounded-full bg-[#0a4f42] px-3 text-[11px] font-semibold text-white hover:bg-[#0a4f42]/90 shadow-2xs"
            >
              <Link href="/scholarMessage" className="flex items-center gap-1.5">
                <span>Chat</span>
      ...
      {/* RIGHT: Announcements & Community Forum */}
      <Card className="flex flex-col justify-between rounded-2xl border-line/80 bg-white shadow-2xs hover:shadow-xs transition-all">
        <CardHeader className="flex flex-row items-center justify-between pb-3 p-5">
          <div className="flex items-center gap-2.5">
